...
/*   Updated: 2022/03/07 15:05:15 by vrigaudy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minitalk.h"

static void	ft_handler_client(int sig, siginfo_t *siginfo, void *context)
{
	(void)context;
	if (sig == SIGUSR2)
	{
		ft_printf("MESSAGE RECEPTION FROM PID %d CONFIRMED\n", siginfo->si_pid);
		exit(EXIT_SUCCESS);
	}
}
...
int	main(int ac, char **av)
{
	...
	i = 0;
	sa.sa_sigaction = &ft_handler_client;
